...
with tf.variable_scope('fc') as scope:
    ...
    # TO DO
    weight = tf.get_variable('weights', [input_features, 1024], initializer=tf.truncated_normal_initializer())
    bias = tf.get_variable('biases', [1024], initializer=tf.constant_initializer(0.0))
    # reshape pool2 to 2 dimensional
    pool2 = tf.reshape(pool2, [-1, input_features])

    # apply relu on matmul of pool2 and w + b
    fc = tf.nn.relu(tf.matmul(pool2, weight) + bias, name='relu')
...
with tf.variable_scope('softmax_linear') as scope:
    ...
    # TO DO
    weight= tf.get_variable('weights', [1024, N_CLASSES], initializer=tf.truncated_normal_initializer())
    bias = tf.get_variable('biases', [N_CLASSES], initializer=tf.random_normal_initializer())
    logits = tf.matmul(fc, weight) + bias
...
with tf.Session() as sess:
    ...
    writer = tf.summary.FileWriter('./graphs_ConvNet', sess.graph)
